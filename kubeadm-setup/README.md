...
## 9) Initialize the First Control Plane
...
The `--upload-certs` flag uploads the shared control-plane certificates in encrypted form, allowing additional control-plane nodes to join the cluster without manually copying certificate files.

If you see `user is not running as root`, run the command again with `sudo`.
...
* Static Pod manifests for:
  ...
  * kube-scheduler
* A bootstrap token
* A discovery token CA certificate hash
* A certificate key
* Join commands for additional control-plane and worker nodes
...
Then you can join worker nodes by running:
...
**At this stage, before installing a CNI plugin, the control-plane node may show a `NotReady` status and CoreDNS pods may remain in a `Pending` state. This is expected because pod networking has not yet been configured.**

### 9.4 Join Additional Control Plane Nodes

Run the control-plane join command generated by kubeadm on each additional control-plane node.
...
After all control-plane nodes have joined:
...
```text
control-plane-01   NotReady
control-plane-02   NotReady
control-plane-03   NotReady
```

### 9.5 Join Worker Nodes

Run the worker join command on each worker node.
...
Verify that all nodes have joined successfully:
